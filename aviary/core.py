    ...
    def evaluate(
        ...
        for inputs, targets_list, *_ in tqdm(data_loader, disable=None if pbar else True):
            ...
            for target_name, targets, output, normalizer in zip(
                ...
                # print('target_name:', target_name, 'targets', targets[:5], 'output', output[:5], 'normalizer.state_dict', normalizer.state_dict())
                ...
                if task == "regression":
                    ...
                    if self.robust:
                        ...
                        # output = torch.clamp(output, min=-1e7, max=1e7) #avoid NaN       
