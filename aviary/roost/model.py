...
class RoostBarlowTwin(BaseModelClass):
    """The Roost model is comprised of a fully connected network
    and message passing graph layers.
    ...
    def forward(
        ...
        if mask_idx is not None:
            crys_fea_masked = crys_fea.clone()
            crys_fea_masked[mask_idx] = 0  # Example masking by setting to zero
        else:
            crys_fea_masked = crys_fea
            
        crys_fea_unmasked = F.relu(self.trunk_nn(crys_fea))
        crys_fea_masked = F.relu(self.trunk_nn(crys_fea_masked))

        # Output from the neural network for both unmasked and masked
        unmasked_output = tuple(output_nn(crys_fea_unmasked) for output_nn in self.output_nns)
        masked_output = tuple(output_nn(crys_fea_masked) for output_nn in self.output_nns)

        return unmasked_output, masked_output
